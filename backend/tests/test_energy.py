...
- Stale data flag propagates from PedestalAPIClient
"""
...
def test_daily_analytics_returns_data(client, admin_headers, marina_id):
    ...
    factory = _mock_factory(get_daily_analytics=(mock_data, False))
    _override(factory)

    r = client.get(f"/api/marinas/{marina_id}/energy/daily", headers=admin_headers)
    _clear()
...
def test_session_summary_aggregation(client, admin_headers, marina_id):
    ...
    factory = _mock_factory(get_session_summary=(mock_summary, False))
    _override(factory)

    r = client.get(f"/api/marinas/{marina_id}/energy/summary", headers=admin_headers)
    _clear()
...
def test_stale_data_flag_propagates(client, admin_headers, marina_id):
    ...
    factory = _mock_factory(get_daily_analytics=(mock_data, True))
    _override(factory)
...
def test_marina_manager_blocked_from_restricted_marina_energy(
    ...
    factory = _mock_factory(get_daily_analytics=([], False))
    _override(factory)
...
def test_active_sessions_endpoint(client, admin_headers, marina_id):
    ...
    factory = _mock_factory(get_active_sessions=(mock_sessions, False))
    _override(factory)

    r = client.get(f"/api/marinas/{marina_id}/sessions/active", headers=admin_headers)
    _clear()
...
def test_pending_sessions_endpoint(client, admin_headers, marina_id):
    ...
    factory = _mock_factory(get_pending_sessions=(mock_sessions, False))
    _override(factory)

    r = client.get(f"/api/marinas/{marina_id}/sessions/pending", headers=admin_headers)
    _clear()
